...
TidyDataSet2 <- merge(TidyDataSet, activityLabels,
      by='activityId',
      all.x=TRUE)

names(TidyDataSet2)[names(TidyDataSet2) == 'activityType.y'] <- 'Activity'

names(TidyDataSet2)[names(TidyDataSet2) == 'activityType.x'] <- 'activityType'

#WRITE TIDY DATASET TO txt file, TidyDataSet.txt
write.table(TidyDataSet2, "TidyDataSet.txt", row.name=FALSE)

#Print variable names for CODEBOOK
str(TidyDataSet2)
